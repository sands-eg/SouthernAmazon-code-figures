...
for j, y in enumerate(lcs):
    ...
    for i in range(1, 7):
        ...
        if fire_slice.shape == reshape_lc.shape:
...
for i in range(6):
    ...
    if i == 5:
        sns.heatmap(plot_data[i]*100, vmin=0, vmax=2, cmap='Reds', ax = axes[i],\
        ...
        cbar.set_label('Mean burned area (% grid cell area)', fontsize = 8)
    else:
        sns.heatmap(plot_data[i]*100,  cmap='Reds', vmin=0, vmax=2,  ax = axes[i], cbar = False)
