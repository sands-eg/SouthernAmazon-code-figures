...
fn = 'R:/Model/Isoprene/Comparison_1800/annual_isop_1800_2012-2014.nc'
...
lons_new = model.lon.values
lats_new = model.lat.values
...
# make new grid
data_new=np.zeros((36, n_lats_new, n_lons_new))+mdi

# now interpolate over lon and lat dimensions
tmp_data=np.zeros((36, n_lats_new, n_lons_old))+mdi
...
# ### new grid
lon_edges_n = np.arange(-180, 180.01, 0.9375*2)
lat_edges_n = np.arange(90, -90.1, -0.625*2)


# initialise variables
n_lons_n = len(lon_edges_n) - 1 # len() works for 1D arrays
n_lats_n = len(lat_edges_n) - 1 

R = 6371.0 # the radius of Earth in km
mdi = -999.99 # no data value

surface_area_earth_n = np.zeros((n_lats_n, n_lons_n)) + mdi

# get lat and lon in radians
lons_rad_n = lon_edges_n*(2*np.pi/360.0)
lats_rad_n = lat_edges_n*(2*np.pi/360.0)

# calculate surface area for lon lat grid 
# surface area = -1 * R^2 * (lon2 - lon1) * (sin(lat2) - sin(lat1)) # lat and lon in radians

for i in range(n_lons_n):
    for j in range(n_lats_n):
        term1 = R**2
        term2 = (lons_rad_n[i+1] - lons_rad_n[i])
        term3 = np.sin(lats_rad_n[j+1]) - np.sin(lats_rad_n[j])
        
        tmp_sa_n = -1*term1*term2*term3 # without -1 surface area comes out negative (lat in different order for MODIS?)
        surface_area_earth_n[j, i] = tmp_sa_n
print('Surface Area of Earth in km2 = ', np.sum(surface_area_earth_n, axis = (0,1)))

burden_new = np.nansum((data_new * (surface_area_earth_n*10**10)), axis=(1,2))
...
isoprene_monthly = xr.DataArray(
    ...
    attrs=dict(
        description="Monthly isoprene on model grid",
        units="mol/cm^2",
    ),
)       

data = isoprene_monthly.rename('isoprene')
# data.to_netcdf('R:/Model/Isoprene/Comparison_1800/cris_isoprene_2012-2014_model_grid.nc')
